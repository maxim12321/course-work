...
void ColumnIterationSolver::LeftColumn(const Matrix& prev_iter,
    ...
    // (0, 0) node
    {
        R = properties_->GetHeatCapacity(0, 0) * properties_->GetDensity(0, 0) +
        ...
        Lambda_z = properties_->GetThermalConductivity(0, 0.25) / (0.5 * properties_->GetDeltaZ(1)) * (prev_iter[0][1] - prev_iter[0][0]);
        tridiagonal[0][0] = 1;
    ...
    // (0, i), i = 1..M, nodes
    for (int k = 1; k <= M_; ++k) {
        tridiagonal[k][0] = properties_->GetDeltaT() * properties_->GetThermalConductivity(0, k - 0.5) / (properties_->GetDeltaZ(k) * properties_->GetDeltaBackZ(k));
        tridiagonal[k][2] = properties_->GetDeltaT() * properties_->GetThermalConductivity(0, k + 0.5) / (properties_->GetDeltaZ(k) * properties_->GetDeltaBackZ(k + 1));
        tridiagonal[k][1] = properties_->GetHeatCapacity(0, k) * properties_->GetDensity(0, k) +
                            tridiagonal[k][0] + tridiagonal[k][2] +
                            properties_->GetDeltaT() * properties_->GetAlpha1() / (0.25 * properties_->GetDeltaX(1));
        tridiagonal[i][1] *= -1;

        Lambda_z = (properties_->GetThermalConductivity(0, k + 0.5) / properties_->GetDeltaBackZ(k + 1) * (prev_iter[0][k + 1] - prev_iter[0][k]) -
                    properties_->GetThermalConductivity(0, k - 0.5) / properties_->GetDeltaBackZ(k) * (prev_iter[0][k] - prev_iter[0][k - 1])) / properties_->GetDeltaZ(k);

        column[k] = properties_->GetHeatCapacity(0, k) * properties_->GetDensity(0, k) * semi_prev_iter[0][k] +
...
void ColumnIterationSolver::MiddleColumn(const Matrix& prev_iter,
    ...
    // (i, 0) node
    {
        R = properties_->GetHeatCapacity(i, 0) * properties_->GetDensity(i, 0) +
            properties_->GetDeltaT() / 0.25 * (properties_->GetThermalConductivity(i, 0.25) / (0.5 * properties_->GetDeltaZ(1) * properties_->GetDeltaZ(1)) +
                                               properties_->GetAlpha3() / properties_->GetDeltaZ(1));

        Lambda_z = properties_->GetThermalConductivity(i, 0.25) / (0.5 * properties_->GetDeltaZ(1)) * (prev_iter[i][1] - prev_iter[i][0]);
...
void ColumnIterationSolver::RightColumn(const Matrix& prev_iter,
    ...
    // (N + 1, 0) node
    {
        ...
        Lambda_z = properties_->GetThermalConductivity(N_, 0.25) / (0.5 * properties_->GetDeltaZ(1)) * (prev_iter[N_ + 1][1] - prev_iter[N_ + 1][0]);
        tridiagonal[0][0] = 1;
    ...
    // (N + 1, M + 1) node
    {
        ...
        column[M_ + 1] =  (properties_->GetHeatCapacity(N_, M_) * properties_->GetDensity(N_, M_) * semi_prev_iter[N_ + 1][M_ + 1] + /* (-) in doc ?*/
                           properties_->GetDeltaT() / (0.25 * properties_->GetDeltaZ(M_)) * Lambda_z +
    ...
    // (N + 1, k), k = 1..M, nodes
    for (int k = 1; k <= M_; ++k) {
        tridiagonal[k][0] = properties_->GetDeltaT() * properties_->GetThermalConductivity(N_, k - 0.5) / (properties_->GetDeltaZ(k) * properties_->GetDeltaBackZ(k));
        tridiagonal[k][2] = properties_->GetDeltaT() * properties_->GetThermalConductivity(N_, k + 0.5) / (properties_->GetDeltaZ(k) * properties_->GetDeltaBackZ(k + 1));
        tridiagonal[k][1] = properties_->GetHeatCapacity(N_, k) * properties_->GetDensity(N_, k) +
                            tridiagonal[k][0] + tridiagonal[k][2] +
                            properties_->GetDeltaT() * properties_->GetAlpha2() / (0.25 * properties_->GetDeltaX(N_));
        tridiagonal[k][1] *= -1;

        Lambda_z = (properties_->GetThermalConductivity(N_, k + 0.5/*(k + 1) in dock*/) / properties_->GetDeltaBackZ(k + 1) * (prev_iter[N_ + 1][k + 1] - prev_iter[N_ + 1][k]) -
                    properties_->GetThermalConductivity(N_, k - 0.5/*(k) in dock*/) / properties_->GetDeltaBackZ(i) * (prev_iter[N_ + 1][k] - prev_iter[N_ + 1][k - 1])) /
                    properties_->GetDeltaZ(k);

        column[i] = properties_->GetHeatCapacity(N_, k) * properties_->GetDensity(N_, k) * semi_prev_iter[N_ + 1][k] +
