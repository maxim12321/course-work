...
void ColumnIterationSolver::LeftColumn(const Matrix& prev_iter,
    ...
    // (0, 0) node
    long double R = properties_->GetHeatCapacity(0, 0, prev_iter) * properties_->GetDensity(0, 0, prev_iter) +
    ...
    long double Lambda_z = properties_->GetThermalConductivity(0, 0.25, prev_iter) / (0.5 * properties_->GetDeltaZ(1)) * (prev_iter[0][1] - prev_iter[0][0]);
    tridiagonal[0][0] = 1;
    ...
    // (0, i), i = 1..M, nodes
    for (int i = 1; i <= M_; ++i) {
        tridiagonal[i][0] = properties_->GetDeltaT() * properties_->GetThermalConductivity(0, i - 0.5, prev_iter) / (properties_->GetDeltaZ(i) * properties_->GetDeltaBackZ(i));
        tridiagonal[i][2] = properties_->GetDeltaT() * properties_->GetThermalConductivity(0, i + 0.5, prev_iter) / (properties_->GetDeltaZ(i) * properties_->GetDeltaBackZ(i + 1));
        tridiagonal[i][1] = properties_->GetHeatCapacity(0, i, prev_iter) * properties_->GetDensity(0, i, prev_iter) +
                            tridiagonal[i][0] + tridiagonal[i][2] +
                            properties_->GetDeltaT() * properties_->GetAlpha1() / (0.25 * properties_->GetDeltaX(1));
        tridiagonal[i][1] *= -1;

        Lambda_z = (properties_->GetThermalConductivity(0, i + 0.5, prev_iter) / properties_->GetDeltaBackZ(i + 1) * (prev_iter[0][i + 1] - prev_iter[0][i]) -
                    properties_->GetThermalConductivity(0, i - 0.5, prev_iter) / properties_->GetDeltaBackZ(i) * (prev_iter[0][i] - prev_iter[0][i - 1])) / properties_->GetDeltaZ(i);

        column[i] = properties_->GetHeatCapacity(0, i, prev_iter) * properties_->GetDensity(0, i, prev_iter) * semi_prev_iter[0][i] +
...
void ColumnIterationSolver::RightColumn(const Matrix& prev_iter,
    ...
    // (N + 1, 0) node
    long double R = properties_->GetHeatCapacity(N_, 0, prev_iter) * properties_->GetDensity(N_, 0, prev_iter) +
    ...
    long double Lambda_z = properties_->GetThermalConductivity(N_, 0.25, prev_iter) / (0.5 * properties_->GetDeltaZ(1)) * (prev_iter[N_ + 1][1] - prev_iter[N_ + 1][0]);
    tridiagonal[0][0] = 1;
    ...
    column[M_ + 1] =  (properties_->GetHeatCapacity(N_, M_, prev_iter) * properties_->GetDensity(N_, M_, prev_iter) * semi_prev_iter[0][M_ + 1] + /* (-) in doc ?*/
                       properties_->GetDeltaT() / (0.25 * properties_->GetDeltaZ(M_)) * Lambda_z +
    ...
    // (N + 1, i), i = 1..M, nodes
    for (int i = 1; i <= M_; ++i) {
        tridiagonal[i][0] = properties_->GetDeltaT() * properties_->GetThermalConductivity(N_, i - 0.5, prev_iter) / (properties_->GetDeltaZ(i) * properties_->GetDeltaBackZ(i));
        tridiagonal[i][2] = properties_->GetDeltaT() * properties_->GetThermalConductivity(N_, i + 0.5, prev_iter) / (properties_->GetDeltaZ(i) * properties_->GetDeltaBackZ(i + 1));
        tridiagonal[i][1] = properties_->GetHeatCapacity(N_, i, prev_iter) * properties_->GetDensity(N_, i, prev_iter) +
                            tridiagonal[i][0] + tridiagonal[i][2] +
                            properties_->GetDeltaT() * properties_->GetAlpha2() / (0.25 * properties_->GetDeltaX(N_));
        tridiagonal[i][1] *= -1;

        Lambda_z = (properties_->GetThermalConductivity(N_, i + 0.5, prev_iter) / properties_->GetDeltaBackZ(i + 1) * (prev_iter[N_ + 1][i + 1] - prev_iter[N_ + 1][i]) -
                    properties_->GetThermalConductivity(N_, i - 0.5, prev_iter) / properties_->GetDeltaBackZ(i) * (prev_iter[N_ + 1][i] - prev_iter[N_ + 1][i - 1])) / properties_->GetDeltaZ(i);

        column[i] = properties_->GetHeatCapacity(N_, i, prev_iter) * properties_->GetDensity(N_, i, prev_iter) * semi_prev_iter[N_ + 1][i] +
